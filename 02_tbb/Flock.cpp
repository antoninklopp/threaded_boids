...
void Flock::moveBoidsToNewPositions() {
	...
	tbb::parallel_for(tbb::blocked_range<size_t>(0, this->boids.size()),[=](const tbb::blocked_range<size_t>& r) {
		for (size_t i = r.begin(); i != r.end(); ++i)
			this->boids[i].computeNewVelocityAndPosition(boids_rule_vectors[i]);
		}
	);
